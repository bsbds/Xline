...
use xline_client::{
    clients::auth::{AuthRoleGrantPermissionRequest, Permission},
    error::Result,
    Client,
};
use xlineapi::{AuthRoleGrantPermissionResponse, Type};

use crate::{utils::printer::Printer, value_enum};
...
pub(super) fn command() -> Command {
    Command::new("grant_perm")
        ...
        .arg(arg!(<perm_type> "The type of the permission").value_parser(value_parser!(PermType)))
        .arg(arg!(<key> "The Key"))
...
pub(super) fn build_request(matches: &ArgMatches) -> AuthRoleGrantPermissionRequest {
    ...
    let perm_type_local = matches.get_one::<PermType>("perm_type").expect("required");
    ...
    let perm_type = match *perm_type_local {
        PermType::Read => Type::Read,
        PermType::Write => Type::Write,
        PermType::Readwrite => Type::Readwrite,
    };
...
pub(super) async fn execute(client: &mut Client, matches: &ArgMatches) -> Result<()> {
    ...
    let resp = client.role_grant_permission(req).await?;
...
/// Printer of role grant permission response
fn print_resp(resp: &AuthRoleGrantPermissionResponse) {
...
mod tests {
    ...
    testcase_struct!(AuthRoleGrantPermissionRequest);
