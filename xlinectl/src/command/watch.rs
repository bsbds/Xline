...
use xline_client::{
    clients::watch::{WatchRequest, Watcher},
    error::ClientError,
    Client,
};
...
pub(crate) async fn execute(client: &mut Client, matches: &ArgMatches) -> Result<()> {
    ...
    if interactive {
        ...
        let (_watcher, mut stream) = client.watch(request).await?;
...
/// execture the command in interactive mode
async fn exec_interactive(client: &mut Client, matches: &ArgMatches) -> Result<()> {
    ...
    loop {
        ...
        macro_rules! failed {
            ($line:expr) => {
                let err = CtlError::InvalidArgs(format!("parse failed in: `{}`", $line));
                eprintln!("{err}");
                continue;
            };
        }
        ...
        match args.next().unwrap() {
            "watch" => {
                ...
                let (new_watcher, mut stream) = client.watch(request).await?;
...
/// Printer of watch response
fn print_resp(resp: &WatchResponse) {
    println!("created: {}", resp.created);
    println!("watch_id: {}", resp.watch_id);
    println!("compact_revision: {}", resp.compact_revision);
    println!("fragment: {}", resp.fragment);
    println!("canceled: {}", resp.canceled);
    println!("cancel_reason: {}", resp.cancel_reason);
