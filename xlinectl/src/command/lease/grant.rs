...
use xline_client::{clients::lease::LeaseGrantRequest, error::Result, Client};
use xlineapi::LeaseGrantResponse;
...
pub(super) async fn execute(client: &mut Client, matches: &ArgMatches) -> Result<()> {
    ...
    let resp = client.lease_grant(request).await?;
...
fn print_resp(resp: &LeaseGrantResponse) {
    Printer::header(resp.header.as_ref());
    println!("id: {}, ttl: {}", resp.id, resp.ttl);
}
