...
use crate::command::{auth, delete, get, lease, lock, put, role, snapshot, txn, user};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    handle_matches!(matches, client, { get, put, delete, txn, lease, snapshot, lock, auth, user, role });
    Ok(())
}
