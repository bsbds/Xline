...
impl WALSegment {
    ...
    pub(super) fn open(mut lfile: LockedFile, size_limit: u64) -> Result<Self, WALError> {
        let mut file = lfile.into_std();
    ...
    {
        let frame_batches = self.read_all::<C, _, _>(WAL::<C>::new())?;
    ...
    fn read_all<C, U, Item>(&mut self, mut decoder: U) -> Result<Vec<Item>, WALError>
    where
