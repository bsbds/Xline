...
use xline_client::{
    types::kv::{Compare, RangeRequest, TxnOp, TxnRequest},
    ClientOptions,
};
...
impl CommandRunner {
    ...
    async fn get_bench(
            ...
            .map(<[usize]>::to_vec)
        {
            let mut client = clients
                .pop()
                .unwrap_or_else(|| unreachable!("should always exist"));
            let val_c = val.clone();
            put_handles.push(tokio::spawn(async move {
                let mut buf = vec![0u8; key_size];
                for i in is {
                    Self::fill_usize_to_buf(&mut buf, i);
                    let _ignore = client.put(buf.clone(), val_c.clone(), None).await?;
                }
        ...
        for (id, mut client) in clients.into_iter().enumerate() {
            ...
            let handle = tokio::spawn(async move {
                ...
                loop {
                    ...
                    let put_op = TxnOp::put(key.clone(), new_val.clone(), None);
                    ...
                    if let Ok(res) = result.as_ref() {
                        if res.succeeded {
                            ...
                            let get = res.responses.first().unwrap_or_else(|| {
                                unreachable!("should contains at least one response")
                            });
                            if let xlineapi::Response::ResponseRange(ref range) = *get
                                .response
                                .as_ref()
                                .unwrap_or_else(|| unreachable!("should contains responses"))
                            {
    ...
    async fn initialze_key(client: &mut BenchClient, key: Vec<u8>, val_size: usize) {
        ...
        let put_op = TxnOp::put(key.clone(), init_value.clone(), None);
        let _ignore = client
