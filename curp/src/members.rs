...
impl ClusterInfo {
    ...
    #[inline]
    #[must_use]
    pub fn new(all_members: HashMap<String, Vec<String>>, self_name: &str) -> Self {
        let mut member_id = 0;
        let mut members = HashMap::new();
        for (name, addrs) in all_members {
            let id = Self::calculate_member_id(&addrs, "", None);
            if name == self_name {
                member_id = id;
            }
    ...
    pub fn address(&self, id: ServerId) -> Option<&Vec<String>> {
        self.members.values().find(|t| t.id == id).map(|t| &t.addrs)
    }
    ...
    #[must_use]
    #[inline]
    pub fn peers(&self) -> HashMap<ServerId, Vec<String>> {
        self.members
            .values()
            .filter(|t| t.id != self.member_id)
    ...
    #[must_use]
    #[inline]
    pub fn all_members(&self) -> HashMap<ServerId, Vec<String>> {
        self.members
            .values()
            .map(|t| (t.id, t.addrs.clone()))
            .collect()
